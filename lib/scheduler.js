...
function Scheduler(connection, opts) {
  ...
  function emit(event, doc, cb) {
    ...
    setTimeout(() => {
      ...
      collection.findAndModify(command, (err, result) => {
        if (err)
  ...
  function poll() {
    ...
    collection.findAndModify(lookup, null, {$set: {status: 'running'}}, (err, result) => {
  ...
  function schedule(details, cb) {
    ...
    const collection = db.collection(collectionName);
    collection.findAndModify(info.query, ['event', 'asc'], info.doc, {new: true}, callback);
  }
