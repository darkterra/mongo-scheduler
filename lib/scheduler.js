...
function Scheduler(connection, options = {}) {
  ...
  if (connection && typeof connection === 'string') {
    ...
    // This normaly fix the issue 10: https://github.com/darkterra/mongo-scheduler/issues/10
  ...
  function emit(event, docs, cb) {
    ...
    setTimeout(() => {
      if (event.cron) {
        const updateDoc = { $set: { status: 'ready', 'conditions.after': parser.parseExpression(event.cron).next() }};
